...
p1 <- ggplot(subset(ivstat,type=="tumour"), aes(x=apt, y=mit))+
  ...
  geom_point(data=subset(dfstat,type=="tumour"),aes(shape=as.character(mouse),color=as.character(day)),size=4)+
  geom_line(data=subset(dfstat,type=="tumour"),aes(group=as.character(mouse)))+
...
p2 <- ggplot(subset(ivstat,type=="CTL"), aes(x=apt, y=mit))+
 facet_grid(cols=vars(ID))+
  geom_point(aes(shape=as.character(mouse),color=as.character(day)),size=2)+
  geom_abline(slope=1,intercept=0,color="red",linetype="dashed")+
  geom_point(data=subset(dfstat,type=="CTL"),aes(shape=as.character(mouse),color=as.character(day)),size=4)+
...
p3 <- ggplot(subset(dfvec,type=="CTL"), aes(x=apt, y=mit))+
  ...
  scale_x_continuous(expression(paste("CTL apoptosis change",(d6-d9))))+
  scale_y_continuous(expression(paste("CTL mitosis change",(d6-d9))))+
  scale_color_manual("day",values=dayz)+
  scale_shape_manual("mouse",values=mouse_shapes)+
  scale_linetype_discrete("")+
  theme_classic(base_size = 14)
...
p4 <- ggplot(subset(dfvec,type=="tumour"), aes(x=apt, y=mit))+
  ...
  geom_point(aes(shape=as.character(mouse),color=as.character(day)),size=4)+
  geom_line(aes(group=as.character(mouse)))+
  scale_x_continuous(expression(paste("TC apoptosis change ",(d6-d9))))+
  scale_y_continuous(expression(paste("TC mitosis change ",(d6-d9))))+
  scale_color_manual("day",values=dayz)+
  scale_shape_manual("mouse",values=mouse_shapes)+
  scale_linetype_discrete("")+
  theme_classic(base_size = 14)
...
if(TRUE){
  ...
  pad_alliv <- egg::ggarrange(p1,p2,p1a,p2a,p4,p3,nrow=3)
...
to_string <- as_labeller(c('1' = paste("ACT (d3, ","OVA^'-'",")",sep=""), 
                           '2' = paste("no~ACT ", " (d3, ","OVA^'+'",")",sep=""),
                           '3' = "ACT-only~(d3)", 
                           '4' = "ACT+mAb~(d3)",
                           '5' = "ACT-only~(d7)", 
...
pg <-ggplot(mdf,aes(x=tmid,y=g,color=treated))+
  ...
  scale_color_discrete("",labels=c("no ACT","ACT"))+
...
p2c <- ggplot(subset(fits,type=="linear"),aes(x=X))+
  ...
  scale_y_continuous(expression(tumour~cell~apoptosis~events~(hour^-1)))
p2c

p2d <- ggplot(subset(fits,type=="mass-action"),aes(x=X))+
  facet_grid(cols=vars(trt),scales = "free",labeller=labz)+
...
df.smm <- rbind(data.frame(CTL=iv.proc$CTL,TC=iv.proc$TC,mitosis=iv.proc$TC*iv.proc$TCm/24,day=iv.proc$day,treatment=iv.proc$treatment,start="d3.s"),
                data.frame(CTL=summary$CTL,TC=summary$TCs,mitosis=summary$mitosis,day=10,treatment=summary$treatment,start="d7.s"))


p2c <- ggplot(iv.proc,aes(x=CTL))+
  geom_ribbon(data=exp_ctl_pois,aes(ymin=conf05,ymax=conf95),alpha=0.1)+
  geom_line(data=exp_ctl_pois,aes(y=exp),color="red")+
  geom_point(size=1.5,aes(color=treatment,y=TCa*TC/24))+
  theme_classic(base_size=20)+
...
p <- ggplot(dat,aes(x=X*0.7,y=Y*0.7,color=type))+
  facet_grid(rows=vars(position),cols=vars(mouse),labeller=lablr)+
...
p <- ggplot(dat,aes(x=X*0.7,y=Slice*7,color=type))+
  facet_grid(rows=vars(position),cols=vars(mouse),labeller=lablr)+
  geom_point(alpha=0.6)+
  scale_color_viridis_d(labels=c("CTL","mitosis"))+
  theme_classic(base_size = 14)+
  scale_x_continuous(expression(X~(paste(mu,m,sep=""))))+
  scale_y_continuous(expression(Z~(paste(mu,m,sep=""))))
p

p <- ggplot(dat,aes(x=Slice*7,y=Y*0.7,color=type))+
  facet_grid(rows=vars(position),cols=vars(mouse),labeller=lablr)+
  geom_point(alpha=0.6)+
  scale_color_viridis_d(labels=c("CTL","mitosis"))+
  theme_classic(base_size = 14)+
  scale_x_continuous(expression(Z~(paste(mu,m,sep=""))))+
  scale_y_continuous(expression(Y~(paste(mu,m,sep=""))))
p
